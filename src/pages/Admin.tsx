                      ...
                      <TableBody>
                        {referralRewards.map(reward => (
                          <TableRow key={reward.id} className="border-slate-700/50 hover:bg-slate-700/20 transition-colors group">
                            ...
                            <TableCell className="p-6">
                              <Badge 
                                variant={reward.status === 'approved' || reward.status === 'claimed' ? 'default' : reward.status === 'pending' ? 'secondary' : 'destructive'}
                                className={`px-3 py-1 font-black text-[10px] uppercase tracking-tighter ${
                                  reward.status === 'pending' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' : 
                                  (reward.status === 'approved' || reward.status === 'claimed') ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                                  'bg-red-500/10 text-red-400 border-red-500/20'
                                }`}
                              >
                                {reward.status === 'claimed' ? 'AUTO-PAID' : reward.status}
                              </Badge>
                            </TableCell>
                            <TableCell className="p-6 text-right">
